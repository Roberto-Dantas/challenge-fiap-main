...
export default function ContentDetailScreen({ navigation, route }: Props) {
  ...
  function shuffleQuestionOptions(question: Question): Question {
    ...
    const options = shuffleQuestions(question.options);
    return { ...question, options, correctOption: options.indexOf(correctAnswer) };
  }

  function startQuiz() {
    setQuizQuestions(shuffleQuestions(filteredQuestions).map(shuffleQuestionOptions));
    setQuizIndex(0);
    setQuizScore(0);
    setSelectedAnswer(null);
    setQuizFinished(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <CreateSummaryModal visible={summaryModalVisible} onClose={() => { setSummaryModalVisible(false); setEditingSummaryId(null); }} heading={editingSummaryId ? "Editar resumo" : "Novo resumo"} initialTitle={summaries.find((summary) => summary.id === editingSummaryId)?.title} initialContent={summaries.find((summary) => summary.id === editingSummaryId)?.content} onCreate={(title, content) => { if (editingSummaryId) updateSummary(editingSummaryId, { title, content }); else addSummary({ topicId, title, content }); }} />
